...
namespace avx
{
	void set(const stm::mat4f& mat, __m256* avxRegister)
	{
	...
	stm::mat4f dot(const stm::mat4f& mat1, const stm::mat4f& mat2)
	{
		...
		memcpy(&arr[0 * 8], (float*)&ans, 8 * sizeof(float));
		ans = _mm256_mul_ps(values1[1], values2[1]);
		memcpy(&arr[1 * 8], (float*)&ans, 8 * sizeof(float));
		return stm::mat4f(arr);
	}

	void setColumnRegisters(const stm::mat4f& mat, __m128* avxRegister)
	{
		for (unsigned int i = 0; i < 4; ++i)
			avxRegister[i] = _mm_set_ps(mat[0][i], mat[1][i], mat[2][i], mat[3][i]);
	}

	void setRowRegisters(const stm::mat4f& mat, __m128* avxRegister)
	{
		for (unsigned int i = 0; i < 4; ++i)
	...
	stm::mat4f multiply(const stm::mat4f& mat1, const stm::mat4f& mat2)
	{
		...
		for (unsigned int i = 0; i < 4; ++i)
		{
			...
			memcpy(&temp[i * 4], (float*)&res, 4 * sizeof(float));
		}

		return { temp };
	}

	__m128 addIndvRegisters(__m128 v0, __m128 v1, __m128 v2, __m128 v3)
	{
	...
	float addIndvRegister(__m128 v)
	{
	...
	__m128 setRegisterValues(int count, const float data[4])
	{
		switch (count)
		{
		case 1:
	...
	__m256 load_data(int count, const float data[8])
	{
		switch (count)
		{
		case 1:
	...
	inline __m256 load_data(const float data[8])
	{
		return _mm256_load_ps(data);
	}

	__m256 addRegisters(__m256 v0, __m256 v1, __m256 v2, __m256 v3, __m256 v4, __m256 v5, __m256 v6, __m256 v7)
	{
	...
	inline __m128 load_data128(const float data[4])
	{
